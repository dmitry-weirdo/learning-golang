...
func longestRepeatingSubstring(s string) int {
	return longestRepeatingSubstring_RabinKarp_Optimized(s) // small optimization for powers pre-computation
}

func longestRepeatingSubstring_RabinKarp_Optimized(s string) int {
	...
	// unlike "1044. Longest Duplicate Substring", we're just returning the maximum length, not the repeating substring itself
	return left - 1
}
...
func test(s string, expectedResult int) { // nodes can be null
	...
	fmt.Printf("Longest repeating substring length: %v \n", result)
...
func test1() {
	test("abcd", 0) // all chars are different
}

func test2() {
	test("abbaba", 2) // "ab" and "ba"
}

func test3() {
	test("aabcaabdaab", 3) // "aab"
}

func main() {
	// 1062. Longest Repeating Substring

	// This is basically the same task as "1044. Longest Duplicate Substring",
	// just the return format is the length of the substring, not the substring itself.
	// Otherwise, it's exactly the same Rabin-Karp implementation
